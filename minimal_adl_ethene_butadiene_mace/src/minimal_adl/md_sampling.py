...
def _resolve_generate_initial_conditions(ml):
    ...
    raise RuntimeError("The current MLatom installation does not expose generate_initial_conditions.")
...
def generate_ts_initial_conditions(
    *,
    config: dict[str, Any],
    round_index: int,
    ...
    ml = import_mlatom()
    seed_path = Path(seed_json_path or config["paths"]["ts_seed_json"]).resolve()
    ...
    initial_molecular_database = generate_initial_conditions(
        ...
        reaction_coordinate_momentum=bool(sampling_cfg.get("reaction_coordinate_momentum", True)),
        random_seed=int(sampling_cfg.get("random_seed", 20260324)),
    )
    ...
    return _save_initial_condition_molecules(
        ...
        source_kind="ts_initial_condition",
        source_label="harmonic_quantum_boltzmann_from_ts_seed",
...
def _load_initial_conditions_for_md(
    *,
    ...
    ml = import_mlatom()
    seed_molecule = ml.data.molecule()
    seed_molecule.load(str(Path(config["paths"]["ts_seed_json"]).resolve()), format="json")
    ...
    initial_molecular_database = generate_initial_conditions(
        ...
        use_hessian=bool(sampling_cfg.get("use_hessian", False)),
        reaction_coordinate_momentum=bool(sampling_cfg.get("reaction_coordinate_momentum", True)),
        random_seed=int(sampling_cfg.get("random_seed", 20260324)) + int(round_index),
    )
...
def run_md_sampling_round(
    *,
    ...
    md_max_time = float(
        maximum_propagation_time if maximum_propagation_time is not None else md_cfg.get("maximum_propagation_time", 150.0)
    )
...
def select_md_frames(
    *,
    ...
    sorted_frames = sorted(
        frames,
        key=lambda item: float(item.get("uncertainty", -1.0) or -1.0),
        reverse=True,
    )
    ...
    selected_output_dir = ensure_dir(Path(config["project_root"]).resolve() / "data" / "raw" / f"round_{round_index:03d}_selected")
    ...
    for frame in uncertain_frames:
        ...
        manifest_entries.append(geometry_record.to_manifest_entry(geometry_path, Path(config["project_root"]).resolve()))
    ...
    summary = {
        ...
        "converged": len(uncertain_frames) / total < float(config["active_learning"].get("convergence_ratio", 0.05))
    ...
    history_path = Path(
        config["paths"].get("active_learning_round_history_file", results_dir / "active_learning_round_history.json")
    ).resolve()
