...
def _ensure_torch_load_compat() -> None:
    """兼容旧版 PyTorch 不支�?`weights_only` 参数的情况�?""
...
def import_mlatom():
    ...
    try:
        ...
        raise RuntimeError(
            "当前环境无法导入 `mlatom`。请先激�?`ADL_env`，并使用 `import mlatom as ml` 方式检查安装�?
        ) from exc
    return ml
...
def _normalize_gaussian_method_name(method_name: str) -> str:
    """把论�?README 常用写法转换�?Gaussian 可识别的关键字写法�?""
...
def _create_mlatom_method(
    ...
    *,
    ...
    # MLatom 3.22 在通用 ml.models.methods(...) 路径下可能提前触�?PySCF 接口导入�?    # 对于�?xTB baseline，优先走 xTB 专用接口，避免把不需要的 PySCF 变成硬依赖�?    if method_name.lower() in {"gfn2-xtb", "gfn1-xtb", "ipea1-xtb"} and not method_config.get("program"):
        try:
            ...
            # 如果专用接口不可用，再回退到通用入口，保留原始行为�?            pass

    # �?Gaussian target，优先走专用接口，并把工作目录固定到样本作业目录，便于保�?.com/.log 做排错�?    if str(program_name).lower() == "gaussian":
...
def build_molecular_database_from_geometry_manifest(manifest_path: str | Path):
    """把未标注的几何池转成 MLatom �?molecular_database�?""
