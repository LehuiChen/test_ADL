...
# 提升 conda 解析稳定性（老环境很有用）
conda install -y -n base conda-libmamba-solver
conda config --set solver libmamba
conda config --set channel_priority flexible

# 基础依赖分阶段安装，减少 solving 卡死概率
...
> 在登录节点 `cuda_available=False` 可能正常；请在 GPU 节点复测。

## 7. 常见坑（你之前踩过的）

### 7.1 `关键环境检查未通过: torchani`
...
`xtb` 不在 `PATH`。重新加载系统环境并检查 `which xtb`。

### 7.3 `关键环境检查未通过: g16`

只在 Gaussian target 下要求。检查 `g16-env.sh` 是否成功加载。

### 7.4 `--expect-gpu` 报 CUDA 不可见

先确认你真的在 GPU 节点，再看 `nvidia-smi`。

### 7.5 `torch` 被偷偷升级成 2.x

通常是某次 `pip install` 引起依赖重解。建议重建干净环境，不要在旧环境硬修。
