...
def main() -> None:
    parser = argparse.ArgumentParser(description="在单个 PBS worker 中批量执行 label 任务。")
    parser.add_argument("--config", required=True, help="YAML 配置文件路径。")
    parser.add_argument("--manifest", required=True, help="该 worker 负责的子 manifest 路径。")
    parser.add_argument("--method-key", required=True, choices=["baseline", "target"], help="方法键名。")
    parser.add_argument("--labels-root", required=True, help="统一 label 根目录。")
    parser.add_argument("--status-file", required=True, help="该 worker 的汇总状态文件路径。")
    parser.add_argument("--local-parallelism", type=int, default=1, help="单个 worker 内部并发样本数。")
    parser.add_argument("--worker-name", default="", help="可选 worker 名称，便于日志排查。")
    ...
    try:
        ...
        print(
            f"[{args.worker_name or 'worker'}] start: method={args.method_key}, "
            f"samples={len(manifest_entries)}, local_parallelism={local_parallelism}"
        )
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=local_parallelism) as executor:
            ...
            for future in concurrent.futures.as_completed(futures):
                ...
                print(
                    f"[{args.worker_name or 'worker'}] {result['sample_id']}: "
