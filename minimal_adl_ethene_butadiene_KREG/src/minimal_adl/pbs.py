...
def _normalize_shell_lines(block: str | Iterable[str] | None) -> list[str]:
    ...
    for line in block:
        if isinstance(line, dict):
            raise TypeError(
                "PBS environment blocks must contain plain shell strings. Quote the entire command if it contains colons."
            )
        text = str(line).strip()
        if text:
    ...
    *,
    ...
    while pending:
        ...
        if time.time() - start_time > timeout_seconds:
            ...
            raise TimeoutError(f"Timed out while waiting for status files:\n{pending_text}")
